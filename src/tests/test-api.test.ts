import WorkerClass  from '../api/_worker';
...
const assertBadRequest = async (endpoint: Endpoint, requestBody: any, expectedError: string) => {
	const response = await worker.fetch(new Request(url(endpoint), { body: requestBody, method: 'POST', headers: { 'content-type': 'application/json', 'authorization': unitTestsAuthHeader } }));
	await waitOnExecutionContext(ctx);

	expect(response.status).toBe(400);
	expect(response.statusText).toBe('Bad Request');
	expect((await response.json() as PreopApiError).Error).toBe(expectedError);
};
const assert200 = async (endpoint: Endpoint, requestBody: any, expectedResponse: string) => {
	...
	expect(await response.text()).toBe(expectedResponse);
};

describe('API', () =>
{
	it('Require application/json', async () => {
		...
		for (const i in urls) {
			const response = await worker.fetch(new Request(urls[i], { method: 'POST', headers: { 'authorization': unitTestsAuthHeader } }));
			await waitOnExecutionContext(ctx);

			expect(response.status).toBe(415);
			expect(response.statusText).toBe('Unsupported Media Type');
			expect((await response.json() as PreopApiError).Error).toBe('Content-Type must be "application/json"');
		};
	});
	...
	it('Test Partially-Bad Request', async () => {
	...
	it('Test predictionsPerIol', async () => {
