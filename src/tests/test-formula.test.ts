...
const assertCloseEnough = (expected: number, actual: number, epsilon: number) => {
	expect(Math.abs(expected - actual)).lessThanOrEqual(epsilon, `Values are not close enough: expected ${expected}, got ${actual}`);
}
const closeEnough = (expected: number | string, actual: number | string) => {
	if (typeof expected === 'number' && typeof actual === 'number') {
		assertCloseEnough(expected, actual, 1E-10);
	} else {
		expect(expected).equals(actual, `Values are not equal: expected ${typeof expected === 'string' ? `"${expected}"` : expected}, got ${typeof actual === 'string' ? `"${actual}"` : actual}`);
	}
}
